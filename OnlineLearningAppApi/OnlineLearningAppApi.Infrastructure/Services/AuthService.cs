...
namespace OnlineLearningAppApi.Infrastructure.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<ResponseTokenDto> AuthenticationAsync(LoginDto loginDto)
        {
            ...
            var t = await GenerateJWTAsync(user.Id, userRoles.ToList(), rt.Value);
        ...
        public async Task<ResponseTokenDto> RefreshTokenAsync(RefreshTokenDto refreshTokenDto)
        {
            ...
            var userRoles = await _userManager.GetRolesAsync(user);
            var response = await GenerateJWTAsync(user.Id, userRoles.ToList(), rtNew.Value);

            return response;

        }

        private Token<User> CreateRefreshToken(string clientId, string userId)
        {
        ...
        public async Task<ResponseTokenDto> GenerateJWTAsync(string userId, List<string> userRoles, string refreshToken)
        {
